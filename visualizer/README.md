...
- `playback.rs`: `PlaybackState { current_frame, fps, looping }` とタイムステップ管理。UIとレンダラーからスレッドセーフに参照できるよう `parking_lot::RwLock`。
...
5. レンダリング: 要求された可視化に応じて `TensorView` を CPU 側で `Vec<(x,y,...)>` へ変換し GPU にアップロード
